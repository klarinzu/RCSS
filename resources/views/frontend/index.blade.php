            ...
            <div class="col">
                <div class="card border h-100 category-card text-center rounded p-2" data-category="${category.id}">
                    <div class="card-body">
                        ...
                        <p class="card-text" style="white-space: pre-line;">${category.body}</p>
                    </div>
                </div>
            </div>
        `;
                                    ...
                                    <div class="col animate-slide-in" style="animation-delay: ${index * 100}ms">
                                        <div class="card border h-100 service-card text-center p-2" data-service="${service.id}">
                                            <div class="card-body">
                                                ...
                                                <p class="card-text mb-1" style="white-space: pre-line;">${service.excerpt}</p>
                                                <p class="card-text" style="white-space: pre-line;">${priceDisplay}</p>
                                            </div>
                                        </div>
                                    </div>
                                `;
                                ...
                                <div class="col animate-slide-in" style="animation-delay: ${index * 100}ms">
                                    <div class="card border h-100 employee-card text-center p-2" data-employee="${employee.id}">
                                        <div class="card-body">
                                            ...
                                            <p class="card-text text-muted" style="white-space: pre-line;">${employee.position || 'Professional'}</p>
                                        </div>
                                    </div>
                                </div>
                            `;
            ...
            function updateSummary() {
                ...
                $("#summary-category").text($(".selected-category-name").text().replace("Selected Category: ", ""));
                $("#summary-service").text($(".selected-service-name").text().replace("Selected Service: ", ""));
                $("#summary-employee").text($(".selected-employee-name").text().replace("Selected Staff: ", ""));
                $("#summary-datetime").text(bookingState.selectedDate + " at " + bookingState.selectedTime);
                $("#summary-duration").text(bookingState.selectedService.duration);
                $("#summary-price").text(bookingState.selectedService.price);
                const notes = $("#customer-notes").val();
                $("#summary-notes").html(notes ? notes.replace(/\n/g, "<br>") : "No notes");
            }
